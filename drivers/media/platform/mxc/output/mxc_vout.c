...
static inline unsigned long get_jiffies(struct timeval *t)
{
    ...
    cur.tv_usec = ts.tv_nsec / NSEC_PER_USEC;
...
static int mxc_vidioc_g_pixelaspect(struct file *file, void *priv,
...
{
	struct mxc_vout_output *vout = priv;

	if (sel->type != V4L2_BUF_TYPE_VIDEO_OUTPUT)
		return -EINVAL;

	if (vout->disp_support_windows) {
		sel->r.left = vout->win_pos.x;
		sel->r.top = vout->win_pos.y;
		sel->r.width = vout->task.output.width;
		sel->r.height = vout->task.output.height;
	} else {
		if (vout->task.output.crop.w && vout->task.output.crop.h) {
			sel->r.left = vout->task.output.crop.pos.x;
			sel->r.top = vout->task.output.crop.pos.y;
			sel->r.width = vout->task.output.crop.w;
			sel->r.height = vout->task.output.crop.h;
		} else {
			sel->r.left = 0;
			sel->r.top = 0;
			sel->r.width = vout->task.output.width;
...
{
	struct mxc_vout_output *vout = priv, *pre_vout;
	struct v4l2_rect *b = &vout->crop_bounds;
	struct v4l2_selection fix_up_sel;
	int ret = 0;

	memcpy(&fix_up_sel, sel, sizeof(*sel));
	...
	if (sel->r.width == 0)
		fix_up_sel.r.width = b->width - b->left;
	if (sel->r.height == 0)
		fix_up_sel.r.height = b->height - b->top;

	if (sel->r.top < b->top)
		fix_up_sel.r.top = b->top;
	if (sel->r.top >= b->top + b->height)
		fix_up_sel.r.top = b->top + b->height - 1;
	if (sel->r.height > b->top - sel->r.top + b->height)
		fix_up_sel.r.height =
			b->top - fix_up_sel.r.top + b->height;

	if (sel->r.left < b->left)
		fix_up_sel.r.left = b->left;
	if (sel->r.left >= b->left + b->width)
		fix_up_sel.r.left = b->left + b->width - 1;
	if (sel->r.width > b->left - sel->r.left + b->width)
		fix_up_sel.r.width =
			b->left - fix_up_sel.r.left + b->width;

	/* stride line limitation */
	fix_up_sel.r.height -= fix_up_sel.r.height % 8;
	fix_up_sel.r.width -= fix_up_sel.r.width % 8;
	if ((fix_up_sel.r.width <= 0) || (fix_up_sel.r.height <= 0) ||
		((fix_up_sel.r.left + fix_up_sel.r.width) >
		 (b->left + b->width)) ||
		((fix_up_sel.r.top + fix_up_sel.r.height) >
		 (b->top + b->height))) {
		v4l2_err(vout->vfd->v4l2_dev, "s_crop err: %d, %d, %d, %d",
			fix_up_sel.r.left, fix_up_sel.r.top,
			fix_up_sel.r.width, fix_up_sel.r.height);
		return -EINVAL;
	}

	/* the same setting, return */
	if (vout->disp_support_windows) {
		if ((vout->win_pos.x == fix_up_sel.r.left) &&
			(vout->win_pos.y == fix_up_sel.r.top) &&
			(vout->task.output.crop.w == fix_up_sel.r.width) &&
			(vout->task.output.crop.h == fix_up_sel.r.height))
			return 0;
	} else {
		if ((vout->task.output.crop.pos.x == fix_up_sel.r.left) &&
			(vout->task.output.crop.pos.y == fix_up_sel.r.top) &&
			(vout->task.output.crop.w == fix_up_sel.r.width) &&
			(vout->task.output.crop.h == fix_up_sel.r.height))
			return 0;
	}
	...
	if (vout->disp_support_windows) {
		vout->task.output.crop.pos.x = 0;
		vout->task.output.crop.pos.y = 0;
		vout->win_pos.x = fix_up_sel.r.left;
		vout->win_pos.y = fix_up_sel.r.top;
		vout->task.output.width = fix_up_sel.r.width;
		vout->task.output.height = fix_up_sel.r.height;
	} else {
		vout->task.output.crop.pos.x = fix_up_sel.r.left;
		vout->task.output.crop.pos.y = fix_up_sel.r.top;
	}
...
static const struct v4l2_ioctl_ops mxc_vout_ioctl_ops = {
	...
	.vidioc_g_pixelaspect				= mxc_vidioc_g_pixelaspect,
