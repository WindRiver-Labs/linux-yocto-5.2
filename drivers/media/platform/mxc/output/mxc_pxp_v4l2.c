...
static int pxp_g_selection(struct file *file, void *priv,
					struct v4l2_selection *sel)
{
	struct pxps *pxp = video_get_drvdata(video_devdata(file));

	if (sel->type != V4L2_BUF_TYPE_VIDEO_OUTPUT_OVERLAY)
		return -EINVAL;

	sel->r.left = pxp->pxp_conf.proc_data.drect.left;
	sel->r.top = pxp->pxp_conf.proc_data.drect.top;
	sel->r.width = pxp->pxp_conf.proc_data.drect.width;
	sel->r.height = pxp->pxp_conf.proc_data.drect.height;

	return 0;
}

static int pxp_s_selection(struct file *file, void *priv,
				struct v4l2_selection *sel)
{
	struct pxps *pxp = video_get_drvdata(video_devdata(file));
	int l = sel->r.left;
