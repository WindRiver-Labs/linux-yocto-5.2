...
static int prpvf_start(void *private)
{
	...
	if (cam->vf_bufs_vaddr[0]) {
		dma_free_coherent(&cam->video_dev->dev, cam->vf_bufs_size[0],
				  cam->vf_bufs_vaddr[0], cam->vf_bufs[0]);
	}
	if (cam->vf_bufs_vaddr[1]) {
		dma_free_coherent(&cam->video_dev->dev, cam->vf_bufs_size[1],
				  cam->vf_bufs_vaddr[1], cam->vf_bufs[1]);
	}
	cam->vf_bufs_size[0] = PAGE_ALIGN(size);
	cam->vf_bufs_vaddr[0] = (void *)dma_alloc_coherent(&cam->video_dev->dev,
							   cam->vf_bufs_size[0],
							   &cam->vf_bufs[0],
							   GFP_DMA |
							   GFP_KERNEL);
	...
	cam->vf_bufs_vaddr[1] = (void *)dma_alloc_coherent(&cam->video_dev->dev,
							   cam->vf_bufs_size[1],
							   &cam->vf_bufs[1],
							   GFP_DMA |
							   GFP_KERNEL);
...
out_4:
	...
	if (cam->rot_vf_bufs_vaddr[0]) {
		dma_free_coherent(&cam->video_dev->dev, cam->rot_vf_buf_size[0],
	...
	if (cam->rot_vf_bufs_vaddr[1]) {
		dma_free_coherent(&cam->video_dev->dev, cam->rot_vf_buf_size[1],
