...
static int prp_enc_setup(cam_data *cam)
{
	...
	if (cam->rotation >= IPU_ROTATE_90_RIGHT) {
		if (cam->rot_enc_bufs_vaddr[0]) {
			dma_free_coherent(&cam->video_dev->dev, cam->rot_enc_buf_size[0],
					  cam->rot_enc_bufs_vaddr[0],
					  cam->rot_enc_bufs[0]);
		}
		if (cam->rot_enc_bufs_vaddr[1]) {
			dma_free_coherent(&cam->video_dev->dev, cam->rot_enc_buf_size[1],
		...
		cam->rot_enc_bufs_vaddr[0] =
		    (void *)dma_alloc_coherent(&cam->video_dev->dev, cam->rot_enc_buf_size[0],
					       &cam->rot_enc_bufs[0],
					       GFP_DMA | GFP_KERNEL);
		if (!cam->rot_enc_bufs_vaddr[0]) {
		...
		cam->rot_enc_buf_size[1] =
		    PAGE_ALIGN(cam->v2f.fmt.pix.sizeimage);
		cam->rot_enc_bufs_vaddr[1] =
		    (void *)dma_alloc_coherent(&cam->video_dev->dev, cam->rot_enc_buf_size[1],
					       &cam->rot_enc_bufs[1],
					       GFP_DMA | GFP_KERNEL);
		if (!cam->rot_enc_bufs_vaddr[1]) {
			dma_free_coherent(&cam->video_dev->dev, cam->rot_enc_buf_size[0],
...
static int prp_enc_enabling_tasks(void *private)
{
	...
	cam->dummy_frame.vaddress = dma_alloc_coherent(&cam->video_dev->dev,
			       PAGE_ALIGN(cam->v2f.fmt.pix.sizeimage),
...
static int prp_enc_disabling_tasks(void *private)
{
	...
	if (cam->dummy_frame.vaddress != 0) {
		dma_free_coherent(&cam->video_dev->dev, cam->dummy_frame.buffer.length,
