...
static int mxc_free_frame_buf(cam_data *cam)
{
	...
	for (i = 0; i < FRAME_NUM; i++) {
		if (cam->frame[i].vaddress != 0) {
			dma_free_coherent(&cam->video_dev->dev, cam->frame[i].buffer.length,
...
static int mxc_allocate_frame_buf(cam_data *cam, int count)
{
	...
	for (i = 0; i < count; i++) {
		cam->frame[i].vaddress =
		    dma_alloc_coherent(&cam->video_dev->dev,
			    ...
			    loff_t *ppos)
{
	...
	v_address[0] = dma_alloc_coherent(&cam->video_dev->dev,
				       PAGE_ALIGN(cam->v2f.fmt.pix.sizeimage),
				       &cam->still_buf[0],
				       GFP_DMA | GFP_KERNEL);

	v_address[1] = dma_alloc_coherent(&cam->video_dev->dev,
				       PAGE_ALIGN(cam->v2f.fmt.pix.sizeimage),
				       &cam->still_buf[1],
				       GFP_DMA | GFP_KERNEL);
...
exit0:
	if (v_address[0] != 0)
		dma_free_coherent(&cam->video_dev->dev, cam->v2f.fmt.pix.sizeimage, v_address[0],
				  cam->still_buf[0]);
	if (v_address[1] != 0)
		dma_free_coherent(&cam->video_dev->dev, cam->v2f.fmt.pix.sizeimage, v_address[1],
				  cam->still_buf[1]);
...
static int init_camera_struct(cam_data *cam, struct platform_device *pdev)
{
	...
	*(cam->video_dev) = mxc_v4l_template;
	...
	cam->video_dev->dev.coherent_dma_mask =
