 ...
 */
static const struct ice_dim ice_rx_profile[] = {
	{2, 10},
	{8, 16},
	{32, 0},
	{96, 0},
 ...
 */
static const struct ice_dim ice_tx_profile[] = {
	{2, 10},
	{8, 16},
	{64, 0},
	{128, 0},
...
static void ice_tx_dim_work(struct work_struct *work)
{
	...
	dim = container_of(work, struct net_dim, work);
	rc = container_of(dim, struct ice_ring_container, dim);
	q_vector = container_of(rc, struct ice_q_vector, tx);

	if (dim->profile_ix >= ARRAY_SIZE(ice_tx_profile))
		dim->profile_ix = ARRAY_SIZE(ice_tx_profile) - 1;

	/* look up the values in our local table */
	itr = ice_tx_profile[dim->profile_ix].itr;
	intrl = ice_tx_profile[dim->profile_ix].intrl;
...
static void ice_rx_dim_work(struct work_struct *work)
{
	...
	if (dim->profile_ix >= ARRAY_SIZE(ice_rx_profile))
		dim->profile_ix = ARRAY_SIZE(ice_rx_profile) - 1;

	/* look up the values in our local table */
	itr = ice_rx_profile[dim->profile_ix].itr;
	intrl = ice_rx_profile[dim->profile_ix].intrl;
...
static void ice_napi_enable_all(struct ice_vsi *vsi)
{
	...
	ice_for_each_q_vector(vsi, q_idx) {
		...
		q_vector->tx.dim.mode = NET_DIM_CQ_PERIOD_MODE_START_FROM_EQE;

		INIT_WORK(&q_vector->rx.dim.work, ice_rx_dim_work);
		q_vector->rx.dim.mode = NET_DIM_CQ_PERIOD_MODE_START_FROM_EQE;
...
static int ice_setup_tc_block(struct net_device *dev,
				struct tc_block_offload *f)
{
	...
	switch (f->command) {
	case TC_BLOCK_BIND:
		return tcf_block_cb_register(f->block, ice_setup_tc_block_cb,
					     np, np, f->extack);
	case TC_BLOCK_UNBIND:
		tcf_block_cb_unregister(f->block, ice_setup_tc_block_cb, np);
		return 0;
	default:
		return -EOPNOTSUPP;
	}
	     ...
	     void *type_data)
{
	switch (type) {
	case TC_SETUP_BLOCK:
		return ice_setup_tc_block(netdev, type_data);
	default:
		return -EOPNOTSUPP;
	}
	return -EOPNOTSUPP;
...
static const struct net_device_ops ice_netdev_ops = {
	...
	.ndo_xsk_async_xmit = ice_xsk_wakeup,
