...
static void ice_tx_dim_work(struct work_struct *work)
{
	...
	WARN_ON(dim->profile_ix >= ARRAY_SIZE(ice_tx_profile));
...
static void ice_rx_dim_work(struct work_struct *work)
{
	...
	WARN_ON(dim->profile_ix >= ARRAY_SIZE(ice_rx_profile));
...
static void ice_init_moderation(struct ice_q_vector *q_vector)
{
	...
	rc->dim.mode = NET_DIM_CQ_PERIOD_MODE_START_FROM_EQE;
	...
	ice_write_itr(rc, tx_dynamic ?
		      ice_tx_profile[rc->dim.profile_ix].itr : rc->itr_setting);
	...
	ice_write_itr(rc, rx_dynamic ? ice_rx_profile[rc->dim.profile_ix].itr :
				       rc->itr_setting);
