...
static int vpu_dec_queue_qbuf(struct file *file, struct queue_data *queue,
				struct v4l2_buffer *buf)
{
	int ret = -EINVAL;
	struct vb2_buffer *vb;
	struct vpu_dev *vpu_device = video_drvdata(file);
	...
	down(&queue->drv_q_lock);
	if (queue->vb2_q_inited)
		ret = vb2_qbuf(&queue->vb2_q, vpu_device->v4l2_dev.mdev, buf);
	up(&queue->drv_q_lock);
		...
		)
{
	...
	ret = vpu_dec_queue_qbuf(file, q_data, buf);
	if (ret) {
		vpu_err("error: %s() return ret=%d\n", __func__, ret);
		return ret;
	}
